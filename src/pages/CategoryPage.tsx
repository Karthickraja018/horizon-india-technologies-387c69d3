        ...
        {loading ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {prods.map((product) => {
              ...
              return (
                <div key={product.id} className="surface-card animate-card-lift overflow-hidden group flex flex-col h-full">
                  <Tooltip delayDuration={120}>
                    <TooltipTrigger asChild>
                      <div className="aspect-square bg-background flex items-center justify-center p-5 border-b border-border image-hover cursor-help">
                        <img src={product.image} alt={product.name} className="w-full h-full object-contain" loading="lazy" width={360} height={360} />
                      </div>
                    </TooltipTrigger>
                    <TooltipContent
                      side="top"
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Link to={`/products/${product.categorySlug}/${product.slug}`} className="btn-outline text-[11px] py-2 px-2 animate-button-scale">
                        Details
                      </Link>
                      <button
                        type="button"
